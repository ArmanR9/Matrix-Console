...
void SnakeGame::update(OS& engine) {
	...
	timeSinceLastMove += engine.getFrame_dT();
	...
	if (engine.Joy.getJoyX() != 0 || engine.Joy.getJoyY() != 0) {
		// Stick is further along x axis than y axis
		if (abs(engine.Joy.getJoyX()) > abs(engine.Joy.getJoyY())) {
			int inputDirX = sign(engine.Joy.getJoyX());
		...
		// Stick is further along y axis than x axis
			...
			int inputDirY = sign(engine.Joy.getJoyY());
			// Dont allow dir to be reversed
			if (inputDirY != -dirY) {
				nextDirY = inputDirY;
				nextDirX = 0;
	...
	for (int i = 0; i < snakeLength; i ++) {
		engine.Scr.setLED((int)x[i], (int)y[i],true);
	}

	// Draw food
	if (foodExists) {
		engine.Scr.setLED(foodX, foodY,true);
	}
	else {
		// Handle food spawning
		timeRemainingToNextFoodSpawn -= engine.getFrame_dT();
